"""
The best thresholds parameters of the Controller.

This reference implementation uses the EagleSim bounding boxes to tune the parameters.
The Kalman implementation from Sort.py is used

Mango discovered parameters:
best parameters: {'max_zoom_size': 0.38, 'no_zoom': 0.38, 'tol_width': 0.10}
"""


file_name = 'Controller_tuner.pickle'
...
## Parameter search space of the thresholds of controller
param_space = {
    'tol_width': uniform(0.05, 0.2),
    'no_zoom': uniform(0.05, 0.4),
    'max_zoom_size': uniform(0.2, 0.3)
}
...
def initialise_kalman(next_state, env_id=0):
    global envs
...
def get_objective(env_id = 0, tol_width = 0.15, no_zoom = 0.25, max_zoom_size=0.40, q=None):
    ...
    max_zoom_size = max(no_zoom, max_zoom_size)
    ...
    for i in range(no_of_episodes):
        ...
        while (not initialized) and init_count <10:
            try:
                ...
                kf = initialise_kalman(next_state, env_id)
                initialized = True
        ...
        while not done:
            ...
            t1 = time.time()
            action = get_action_tuner(next_state, kf, env, tol_width, no_zoom, max_zoom_size)
    ...
    print(round(tol_width,2), round(no_zoom,2), round(max_zoom_size,2), ': Reward:', int(total_obj_reward/no_of_episodes), ' steps: ', int(local_steps/(no_of_episodes*20)),  ' time taken:', int(sum(time_taken)*1000.0/len(time_taken)))

    q.put(total_obj_reward/no_of_episodes)
...
def objfunc(args_list):
    ...
    for i in range(batch_size):
        ...
        tol_width = hyper_par['tol_width']
        no_zoom = hyper_par['no_zoom']
        max_zoom_size = hyper_par['max_zoom_size']

        p = Process(target=get_objective, args=(i, tol_width, no_zoom, max_zoom_size, q))
        p.start()
        ps.append(p)
